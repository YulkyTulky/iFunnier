...
- (void)copyBearerToken {
	...
	// No token found
	UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"No Token Found!" message:@"You may need to open iFunny for a few seconds so iFunnier can retrieve the token." preferredStyle:UIAlertControllerStyleAlert];
...
- (void)uninstalliFunny {
		...
		UIAlertAction *yesAction = [UIAlertAction actionWithTitle:@"Yes" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
			[[NSDistributedNotificationCenter defaultCenter] postNotificationName:@"com.yulkytulky.ifunnier/uninstallifunny" object:nil userInfo:nil];
...
- (void)github {
	...
	[[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"https://github.com/YulkyTulky/iFunnier"] options:@{} completionHandler:nil];

}

- (void)discord {
	
	[[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"https://discord.gg/pm2zjMhRgq"] options:@{} completionHandler:nil];
