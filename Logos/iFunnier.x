...
- (instancetype)initWithNetworkClient:(IFNetworkClientImpl *)client {
	...
	NSString *ifunnyDocumentsDirectory = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
	[[[client authorizationHeader] stringByReplacingOccurrencesOfString:@"Bearer " withString:@""] writeToFile:[NSString stringWithFormat:@"%@/%@", ifunnyDocumentsDirectory, @"ifunnierbearertoken"] atomically:YES encoding:NSUTF8StringEncoding error:nil];
	return %orig;
...
static void loadPrefs() {
	...
	NSDictionary *preferences = [[NSDictionary alloc] initWithContentsOfFile:@"/var/mobile/Library/Preferences/com.yulkytulky.ifunnierpreferences.plist"];
...
%ctor {
	...
	loadPrefs();
	CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), NULL, (CFNotificationCallback)loadPrefs, CFSTR("com.yulkytulky.ifunnierpreferences/saved"), NULL, CFNotificationSuspensionBehaviorCoalesce);
